...
export const Drawer = () => {
  return (
    <div 
      className="offcanvas offcanvas-start fixed bottom-0 flex flex-col max-w-full 
        bg-white invisible bg-clip-padding shadow-sm outline-none transition duration-300 
        ease-in-out text-gray-700 top-0 left-0 border-none w-72" 
      tabIndex={-1} 
      id="offcanvasExample" 
      aria-labelledby="offcanvasExampleLabel"
    >
      <div className="offcanvas-header flex items-center justify-between p-4 bg-gray-800">
        <div className="px-8 pt-3">
          <img src={Logo} alt="logo-kompetegram" />
        </div>
      </div>
      <div className="offcanvas-body flex-grow p-1 overflow-y-auto bg-gray-800">
        <ul className="relative">
          <li className="relative px-2">
            <Link 
              to="/dashboard/stats" 
              className="flex uppercase mt-1.5 font-medium items-center text-sm py-4 px-6 h-12 
                font-open-sans overflow-hidden text-gray-50 text-ellipsis whitespace-nowrap 
                rounded hover:text-gray-900 hover:bg-blue-100 hover:font-semibold 
                hover:rounded-lg transition duration-300 ease-in-out" 
